# -*- coding: utf-8 -*-
"""5 环境和社会 下 5 本书（City/Dungeon/Frost/Sand/Storm）噪音页结构化重排。

复用 clean_cw.py 的全部去噪逻辑（normalize / 表格紧凑化 / 全角转半角 /
智能引号与弯引号归一 / 自检），仅将目标目录从单本 CW 改为 5 本循环遍历。
...
处理内容（同 clean_cw.py）：
  ...
  4. 全角 ASCII 转半角；智能引号实体/弯引号 -> 半角直引号；全角 ＆ -> &amp;

安全设计：内容丢失自检（基线 normalize 后比可见字符数）；--apply 前自动备份；干净源跳过。

用法：
  python clean_5books.py            # 干跑
  python clean_5books.py --apply    # 写入（先备份）
"""
import datetime
import html
import os
...
BOOKS = [
    os.path.join(P.WIKI_TIDDLERS, "5 环境和社会", b)
    for b in ["City城市风貌", "Dungeon地城风光", "Frost霜燃之书", "Sand沙暴之书", "Storm风暴之书"]
]
WORK = P.WORK
...
def process(apply=False):
    ...
    stats = {"files": 0, "changed": 0, "p": 0, "tb": 0, "ta": 0, "skipped": 0}
    lost = []
    samples = []
    for path in files:
        ...
        if len(samples) < 2 and new_body != body.strip():
            samples.append((os.path.relpath(path, P.WIKI_TIDDLERS), new_body[:600]))
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(WORK, f"backup_5books_clean_{ts}")
        os.makedirs(bak, exist_ok=True)
        for d in BOOKS:
            for root, dirs, fs in os.walk(d):
                dirs[:] = [x for x in dirs if not x.endswith(".files")]
                rel = os.path.relpath(root, P.WIKI_TIDDLERS)
                for x in fs:
                    if x.lower().endswith(".tid"):
