#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""为 3 完美系列/CM完美巫师 追加 final_mapping 行（若已存在则自动跳过）。

背景：CM 是 WinCHM 源，可能不在 Contents.hhc / hhc_mapping.csv 中。
不能直接重跑 build_mapping.py（会覆盖现有已搬书），故仅把 CM 源目录下所有
.htm/.html 生成映射行追加到 final_mapping.csv，并做跨书/书内消歧。

Tiddler标题 = build_title(文件名去扩展名, 路径分段) -> "X (CM-完美巫师)"
标签       = [[3 完美系列]] [[CM完美巫师]]
"""
import csv
import os
...
CM_REL = r"3 完美系列\CM完美巫师"
SRC_DIR = os.path.join(P.ROOT, CM_REL)
OUT_CSV = bm.OUT_CSV
