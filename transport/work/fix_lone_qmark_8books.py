# -*- coding: utf-8 -*-
"""修复 8 战役引钩 3 本书中单个 '?' 乱码（源混编码致中文标点/英文撇号/表格前导标记解码失败）。

仅限这 3 本书（本次新搬运、已确认为乱码）。不碰其他书——PHB/MM5/PHB2 等书中的
'?' 多为真问句，误改会破坏内容。
...
BOOKS = ["WoL传古武器", "EoE邪恶典范", "EE上古邪物"]
TS = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
BAK = os.path.join(BASE, "backups", "wiki_loneqmark_8books_" + TS)
...
def main():
    ...
    files_hit = []
    for b in BOOKS:
        src = os.path.join(BASE, "wiki", "tiddlers", "8 战役引钩", b)
