# -*- coding: utf-8 -*-
"""MM4怪物图鉴4 噪音页结构化重排（复用 clean_mm3.py 思路，针对 MM4 表格字段化）。
...
MM3 = os.path.join(P.WIKI_TIDDLERS, PREFIX)  # 变量名沿用，指向 MM4 目录

# 含怪物词条的子目录（噪音主要在此）
TARGET_DIRS = [
    "怪物",
    "怪物\\暗爪蜥蜴人", "怪物\\豺狼人", "怪物\\恶魔", "怪物\\恶魔蜂巢",
    "怪物\\风刃", "怪物\\吉斯洋基人", "怪物\\精灵,卓尔",
    "怪物\\魔鬼（尤格罗斯魔）", "怪物\\其它新增怪物", "怪物\\蛇人",
    "怪物\\提亚玛特的邪龙末裔", "怪物\\邪恶元素化身", "怪物\\泽恩相关",
]
# 顶层 tid（怪物专长/挑战等级表等）按需另判，本次先处理含数据表的子目录
...
def process(rel_dir, apply=False):
    ...
    for f in files:
        ...
        if f in ("Aboleth 深渊魔鱼.tid", "Ambush Drake 伏龙兽.tid", "Dinosaur 恐龙.tid"):
            samples.append((f, len(body), len(new_body), new_body[:1600]))
        if apply:
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(P.WORK, f"backup_mm4_clean_{ts}")
