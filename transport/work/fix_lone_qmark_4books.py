# -*- coding: utf-8 -*-
"""修复 4 阵营和位面 4 本书中单个 '?' 乱码（源混编码致中文标点/英文撇号/表格前导标记解码失败）。

仅限这 4 本书（本次新搬运、已确认为乱码）。不碰其他书——PHB/MM5/PHB2 等书中的
'?' 多为真问句，误改会破坏内容。
...
BOOKS = ["BoED崇善之书", "FC1深渊堕群", "FC2九狱君王", "PlH位面手册"]
TS = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
BAK = os.path.join(BASE, "backups", "wiki_loneqmark_4books_" + TS)
...
def main():
    ...
    files_hit = []
    for b in BOOKS:
        src = os.path.join(BASE, "wiki", "tiddlers", "4 阵营和位面", b)
