# -*- coding: utf-8 -*-
"""8 战役引钩 下 3 本书（WoL传古武器/EoE邪恶典范/EE上古邪物）噪音页结构化重排。

复用 clean_cw.py 的全部去噪逻辑（normalize / 表格紧凑化 / 全角转半角 /
智能引号与弯引号归一 / 自检），仅将目标目录从单本 CW 改为 3 本循环遍历。
...
用法：
  python clean_8books.py            # 干跑
  python clean_8books.py --apply    # 写入（先备份）
"""
import datetime
import html
import os
...
BOOKS = [
    os.path.join(P.WIKI_TIDDLERS, "8 战役引钩", b)
    for b in ["WoL传古武器", "EoE邪恶典范", "EE上古邪物"]
]
WORK = P.WORK
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(WORK, f"backup_8books_clean_{ts}")
