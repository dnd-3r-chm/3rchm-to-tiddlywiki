#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""为 3 完美系列/CPsi完美灵能 追加 final_mapping 行（若已存在则自动跳过）。

背景：CPsi 是 WinCHM 源，可能不在 Contents.hhc / hhc_mapping.csv 中。
不能直接重跑 build_mapping.py（会覆盖现有已搬书），故仅把 CPsi 源目录下所有
...
CPSI_REL = r"3 完美系列\CPsi完美灵能"
SRC_DIR = os.path.join(P.ROOT, CPSI_REL)
OUT_CSV = bm.OUT_CSV
