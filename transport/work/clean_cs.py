# -*- coding: utf-8 -*-
"""CS完美恶徒 噪音页结构化重排（复用 clean_cd.py 的 normalize/表格紧凑化/自检思路）。

背景：CS 源与 CC/CD/CM/CPsi 同源（Word/WinCHM 导出），满页带 Word 噪音：
...
安全设计：
  ...
  2. --apply 前自动备份到 work/backup_cs_clean_<时间戳>
  3. 干净源（无噪音标记）直接跳过

用法：
  python clean_cs.py            # 干跑，输出统计与样本
  python clean_cs.py --apply    # 实际写入（先自动备份到 work/）
"""
import datetime
import html
import os
...
CS = os.path.join(P.WIKI_TIDDLERS, "3 完美系列", "CS完美恶徒")
WORK = P.WORK
...
def process(apply=False):
    files = []
    for root, dirs, fs in os.walk(CS):
    ...
    for path in files:
        ...
        if abs(len(ta) - len(tb)) > max(30, len(tb) * 0.01):
            lost.append((os.path.relpath(path, CS), len(tb), len(ta)))
        if len(samples) < 2 and new_body != body.strip():
            samples.append((os.path.relpath(path, CS), new_body[:600]))
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(WORK, f"backup_cs_clean_{ts}")
        os.makedirs(bak, exist_ok=True)
        for root, dirs, fs in os.walk(CS):
            dirs[:] = [x for x in dirs if not x.endswith(".files")]
            rel = os.path.relpath(root, CS)
            for x in fs:
                if x.lower().endswith(".tid"):
