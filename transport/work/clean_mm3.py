# -*- coding: utf-8 -*-
"""MM3怪物图鉴3 噪音页结构化重排（参照 clean_xph.py 思路，针对 MM3 表格字段化）。
...
PREFIX = "1 核心补充书籍\\MM3怪物图鉴3"
MM3 = os.path.join(P.WIKI_TIDDLERS, PREFIX)

# 含怪物词条的子目录（噪音主要在此）
TARGET_DIRS = ["怪物", "动物伙伴与乘骑", "玩家可选", "模板"]
# 顶层 tid（怪物专长/挑战等级表等）按需另判，本次先处理含数据表的四个子目录
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(P.WORK, f"backup_mm3_clean_{ts}")
