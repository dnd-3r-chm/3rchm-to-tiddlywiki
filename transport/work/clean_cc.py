# -*- coding: utf-8 -*-
"""CC完美斗士 噪音页结构化重排（复用 clean_cad.py 的 normalize/表格紧凑化/自检思路）。

背景：CC 源与 CAd 同源（Word/WinCHM 导出），满页带 Word 噪音：
      <span style='FONT-SIZE: 11pt; FONT-FAMILY:"微软雅黑"; mso-bidi-*'>、<span lang=EN-US>、
      <div> 包裹、<o:p>、`<?xml:namespace>`、`&nbsp;`、跨行属性。
      此外 CC 还被 DND3R 在线勘误站注入了 <!-- DND3R-FEEDBACK-BEGIN/END --> 反馈入口区块，
      需整体删除（离线 wiki 无用且含外链）。

与 clean_cad.py 的区别：
  **不做「物品名 -> <h4>」提升**。CC 主体是异质结构（大量表格、列表项、法术/专长条目），
  <b>名称(EN)</b> 多为列表项而非章节标题，提升成 h4 会造成错误层级。仅保留原标题与段落结构。
  额外：删除 DND3R-FEEDBACK 区块；清除 HTML 实体引号 &quot;/&#39;。
...
安全设计：
  ...
  2. --apply 前自动备份到 work/backup_cc_clean_<时间戳>
  3. 干净源（无噪音标记）直接跳过

用法：
  python clean_cc.py            # 干跑，输出统计与样本
  python clean_cc.py --apply    # 实际写入（先自动备份到 work/）
"""
import datetime
import html
import os
...
CC = os.path.join(P.WIKI_TIDDLERS, "3 完美系列", "CC完美斗士")
WORK = P.WORK
...
def normalize(body):
    ...
    # 删除 DND3R 在线勘误站注入的反馈入口区块（含 BEGIN/END 注释及其间的 div/a），
    # 必须在删通用注释之前做，否则注释被删后中间的 div/a 会残留
    body = re.sub(
...
def _emit_para(inner):
    """把片段压平为一个或多个 <p>：<br> 视作段落分隔（CC 不做 h4 提升，保持原结构）。"""
...
def process(apply=False):
    files = []
    for root, dirs, fs in os.walk(CC):
    ...
    for path in files:
        ...
        if new_body != body.strip():
            stats["changed"] += 1
        # 自检：基线同样过 normalize 后比对可见字符（比对前 html.unescape 展开实体，
        # 避免 &quot;(6)->"(1) 等长替换造成的误报）
        ...
        if abs(len(ta) - len(tb)) > max(30, len(tb) * 0.01):
            lost.append((os.path.relpath(path, CC), len(tb), len(ta)))
        if len(samples) < 2 and new_body != body.strip():
            samples.append((os.path.relpath(path, CC), new_body[:600]))
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(WORK, f"backup_cc_clean_{ts}")
