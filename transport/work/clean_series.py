# -*- coding: utf-8 -*-
"""6 种族书 / 7 扩展全新体系 / 9 世设 三系列（46 本，2778 页）噪音页结构化重排。

复用 clean_cw.py 的全部去噪逻辑（normalize / 表格紧凑化 / 全角转半角 /
智能引号与弯引号归一 / 自检），仅将目标目录改为三系列根目录循环遍历。
...
用法：
  python clean_series.py            # 干跑
  python clean_series.py --apply    # 写入（先备份）
"""
import datetime
import html
import os
...
BOOKS = [
    os.path.join(P.WIKI_TIDDLERS, "6 种族书"),
    os.path.join(P.WIKI_TIDDLERS, "7 扩展全新体系"),
    os.path.join(P.WIKI_TIDDLERS, "9 世设"),
]
WORK = P.WORK
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(WORK, f"backup_series_clean_{ts}")
