#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""为 3 完美系列/CAr完美奥术 追加 final_mapping 行。

背景：CAr 是 DND3R 在线勘误版源，不在 Contents.hhc / hhc_mapping.csv 中，
因此 final_mapping.csv 也没有它。不能直接重跑 build_mapping.py（会覆盖
现有已搬书：CAd/MM/XPH 等），故仅把 CAr 源目录下所有 .htm/.html 生成映射行
追加到 final_mapping.csv，并做跨书/书内消歧。

Tiddler标题 = build_title(文件名去扩展名, 路径分段) -> "X (CAr-完美奥术)"
标签       = [[3 完美系列]] [[CAr完美奥术]]
"""
import csv
import os
...
CAr_REL = r"3 完美系列\CAr完美奥术"
SRC_DIR = os.path.join(P.ROOT, CAr_REL)
OUT_CSV = bm.OUT_CSV
