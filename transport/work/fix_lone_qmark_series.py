# -*- coding: utf-8 -*-
"""修复 6 种族书 / 7 扩展全新体系 / 9 世设 三系列中单个 '?' 乱码（源混编码致中文标点/英文撇号/表格前导标记解码失败）。

仅限这三系列（本次新搬运、已确认为乱码）。不碰其他书——PHB/MM5/PHB2 等书中的
'?' 多为真问句，误改会破坏内容。
...
BOOKS = [
    os.path.join(BASE, "wiki", "tiddlers", "6 种族书"),
    os.path.join(BASE, "wiki", "tiddlers", "7 扩展全新体系"),
    os.path.join(BASE, "wiki", "tiddlers", "9 世设"),
]
TS = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
BAK = os.path.join(BASE, "backups", "wiki_loneqmark_series_" + TS)
    ...
    for b in BOOKS:
        for root, _, fs in os.walk(b):
            for f in fs:
                ...
                new = fix_text(s)
                if new != s:
                    files_hit.append((p, os.path.relpath(p, b), s, new))
