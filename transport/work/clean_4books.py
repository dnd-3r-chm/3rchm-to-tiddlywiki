# -*- coding: utf-8 -*-
"""4 阵营和位面 下 4 本书（BoED/FC1/FC2/PlH）噪音页结构化重排。

复用 clean_cw.py 的全部去噪逻辑（normalize / 表格紧凑化 / 全角转半角 /
智能引号与弯引号归一 / 自检），仅将目标目录从单本 CW 改为 4 本循环遍历。
...
用法：
  python clean_4books.py            # 干跑
  python clean_4books.py --apply    # 写入（先备份）
"""
import datetime
import html
import os
...
BOOKS = [
    os.path.join(P.WIKI_TIDDLERS, "4 阵营和位面", b)
    for b in ["BoED崇善之书", "FC1深渊堕群", "FC2九狱君王", "PlH位面手册"]
]
WORK = P.WORK
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(WORK, f"backup_4books_clean_{ts}")
