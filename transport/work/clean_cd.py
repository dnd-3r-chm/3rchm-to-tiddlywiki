# -*- coding: utf-8 -*-
"""CD完美神力 噪音页结构化重排（复用 clean_cc.py 的 normalize/表格紧凑化/自检思路）。

背景：CD 源与 CC 同源（Word/WinCHM 导出），满页带 Word 噪音：
      <span style='FONT-SIZE: 11pt; FONT-FAMILY:"微软雅黑"; mso-bidi-*'>、<span lang=EN-US>、
      <div> 包裹、<o:p>、`<?xml:namespace>`、`&nbsp;`、跨行属性。
      此外 CD 也被 DND3R 在线勘误站注入了 <!-- DND3R-FEEDBACK-BEGIN/END --> 反馈入口区块
      （129 个源文件全部含该区块），需整体删除（离线 wiki 无用且含外链）。

与 clean_cc.py 完全一致的处理：
...
安全设计：
  ...
  2. --apply 前自动备份到 work/backup_cd_clean_<时间戳>
  3. 干净源（无噪音标记）直接跳过

用法：
  python clean_cd.py            # 干跑，输出统计与样本
  python clean_cd.py --apply    # 实际写入（先自动备份到 work/）
"""
import datetime
import html
import os
...
CD = os.path.join(P.WIKI_TIDDLERS, "3 完美系列", "CD完美神力")
WORK = P.WORK
...
def _emit_para(inner):
    """把片段压平为一个或多个 <p>：<br> 视作段落分隔（不做 h4 提升，保持原结构）。"""
...
def process(apply=False):
    files = []
    for root, dirs, fs in os.walk(CD):
    ...
    for path in files:
        ...
        if new_body != body.strip():
            stats["changed"] += 1
        # 自检：基线过 normalize 后比对可见字符（比对前 html.unescape 展开实体）
        ...
        if abs(len(ta) - len(tb)) > max(30, len(tb) * 0.01):
            lost.append((os.path.relpath(path, CD), len(tb), len(ta)))
        if len(samples) < 2 and new_body != body.strip():
            samples.append((os.path.relpath(path, CD), new_body[:600]))
...
def main():
    ...
    if apply:
        ...
        bak = os.path.join(WORK, f"backup_cd_clean_{ts}")
        os.makedirs(bak, exist_ok=True)
        for root, dirs, fs in os.walk(CD):
            dirs[:] = [x for x in dirs if not x.endswith(".files")]
            rel = os.path.relpath(root, CD)
            for x in fs:
                if x.lower().endswith(".tid"):
